...
public class ChatForm extends javax.swing.JFrame {
    ...
    if (nome.isEmpty() || msg.isEmpty() || peer == null) return;

    peer.enviar(nome + ": " + cripto.Criptografia.criptografar(msg));
    jTextChat.append(nome + ": " + msg + "\n");
    ...
    private void btnConectarActionPerformed(java.awt.event.ActionEvent evt) {                                            
        ...
        peer = new TCPPeer(porta);

        jTextChat.append("Iniciando listener na porta " + porta + "...\n");

        new Thread(() -> {
            peer.iniciarListener();
            SwingUtilities.invokeLater(() -> jTextChat.append("Peer conectado ao listener.\n"));

            try {
                String msg;
                while ((msg = peer.receber()) != null) {
                    int i = msg.indexOf(":");
                    if (i != -1) {
                        String cab = msg.substring(0, i + 1);
                        String corpo = msg.substring(i + 1).trim();
                        corpo = cripto.Criptografia.descriptografar(corpo);
                        String textoFinal = cab + " " + corpo;
                        SwingUtilities.invokeLater(() -> jTextChat.append(textoFinal + "\n"));
                    } else {
                        SwingUtilities.invokeLater(() -> jTextChat.append(msg + "\n"));
                    }
                }
            } catch (IOException e) {
                SwingUtilities.invokeLater(() -> jTextChat.append("Conexão encerrada.\n"));
            }
        }).start();
